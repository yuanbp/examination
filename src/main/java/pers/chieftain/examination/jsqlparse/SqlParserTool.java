package pers.chieftain.examination.jsqlparse;
...
public class SqlParserTool {
    ...
    public static void main(String[] args) throws JSQLParserException {
        ...
        if (sqlType.equals(SqlType.SELECT)) {
            Select statement = (Select) SqlParserTool.getStatement(sql);
            getFromItem(statement.getSelectBody());
            String tableAlias = ((PlainSelect) statement.getSelectBody()).getFromItem().getAlias().getName();
            EqualsTo equalsTo = new EqualsTo();
            equalsTo.setLeftExpression(new Column("user_age"));
            equalsTo.setRightExpression(new LongValue(22));
            Expression expression = ((PlainSelect) statement.getSelectBody()).getWhere();
            AndExpression andExpression = new AndExpression(equalsTo,expression);
            ((PlainSelect) statement.getSelectBody()).setWhere(andExpression);
